    ...
    getBindings() {
            ...
            attributes: [
                {
                    ...
                    onGet: () => {
                        ...
                        return (
                          val && settings.contentTypes[val].usesLengthAttribute
                        ) ? 'true' : 'false';
                    },
                },
            ],
            ...
            attributes: [
                {
                    ...
                    onGet: () => {
                        ...
                        return (
                          val &&
                          settings.contentTypes[val].usesPitchSystem
                        ) ? 'true' : 'false';
                    },
                },
            ],
        ...
        bindings[uiElements.pubDateGroup] = {
            ...
            update: ($el, value) => {
                ...
                if (newMode !== '') {
                    const modeConfig = settings.dateGranularities[value];
                    ...
                    if ((currentValue !== '') && (oldMode !== '')) {
                        const oldModeConfig = settings.dateGranularities[oldMode];
                        const initialDate = settings.moment(
                            currentValue,
                            oldModeConfig.format[0]
                        );
                    ...
                    if (addCurrentDate === true) {
                        const newStartFormatted = settings.moment()
                                    .startOf(modeConfig.rounding)
                                    .format(modeConfig.format[0]);
                    ...
                    const singleDateMode = (
                        _.has(settings.datePickerOptions[value], 'singleDate')
                    ) && (
                        settings.datePickerOptions[value].singleDate === true
                    );
                        ...
                        _.defaults(
                            {
                                getValue: () => {
                                    if (ui.pubDateField.val()) {
                                        const startDate = settings.moment(
                                            ui.pubDateField.val(),
                                            modeConfig.format[0]
                                        );
                                ...
                                setValue: (formattedValue) => {
                                    ...
                                    ui.pubDateField.val(
                                        startDate.format(modeConfig.format[0])
                                    );
        ...
        bindings[uiElements.pubDateField] = {
            ...
            update($el, value, mdl) {
                ...
                if (_.contains(['m', 'w', 'd', 't'], mode)) {
                    $el.val(
                        settings.moment.tz(value[0], 'America/Chicago')
                                        .format(
                                            settings.dateGranularities[mode].format[0]
                                        )
                    );
                }
            },
        ...
        bindings[uiElements.pubTimeGroup] = {
            ...
            update($el, value) {
                ...
                if (newMode === 't') {
                    ...
                    if ((currentValue !== '')) {
                        const initialTime = settings.moment(
                            currentValue,
                            modeConfig.format[1]
                        );
                    ...
                    ui.pubTimeField.timeDropper(
                        _.defaults(
                            {
                                fetchTime: () => {
                                    const localizedDate = settings.moment(
                                        ui.pubTimeField.val(),
                                        'h:mm a'
                                    );
                                ...
                                putTime: (s) => {
                                    const formattedTime = settings.moment(
                                        s,
                                        'h:mm a',
                                        'en'
        ...
        bindings[uiElements.pubTimeField] = {
            ...
            update($el, value, mdl) {
                ...
                if (mode === 't') {
                    $el.val(
                        settings.moment.tz(value[0], 'America/Chicago')
                            .format(settings.dateGranularities[mode].format[1])
                    );
                }
            },
            getVal($el) {
                if (model.get('publishDateResolution') === 't') {
                    ...
                    return [
                        ui.pubDateResolution.val(),
                        [
                            model.generateFormattedPublishDate()[0],
                            $el.val(),
                        ].join(' '),
                    ];
                }
